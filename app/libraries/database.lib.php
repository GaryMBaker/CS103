...
class Database {
	...
	private $model;
	private $eachFunc;
	private $eachFuncArgs = [];
	public $get_method = 'get_many';
	...
	public function where_or($data, $value = null, $use_quotes = true) {
		if(is_null($value)) {
			return $this->where($data, $use_quotes, true);
		} else {
	...
	public function each($func, $args) {
		$this->eachFunc = $func;
		$this->eachFuncArgs = $args;
	...
	public function set_get_method($method) {
	...
	public function get_many() {
		...
		$assoc = $this->assoc($this->run($q));

		if($this->model) {
			$models = $this->wrapModel($assoc);

			if(!is_null($this->eachFunc)) {
				foreach($models as $model) {
					$merged = array_merge([$model], $this->eachFuncArgs);

					call_user_func_array($this->eachFunc, $merged);
					// $this->eachFunc($model);
				}
			}
		...
		if(is_array($data)) {
			foreach($data as $field => $value) {
				...
				if(!strpos($this->where, 'WHERE')) {
					...
					// $this->where .= ' WHERE '.$field.' LIKE '.$quotes.$value.$quotes.' ';
	...
	private function wrapModel($arr) {
		$newArr = [];
